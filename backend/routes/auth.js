...
router.post(
  ...
  [
    ...
    body("phone").optional().trim().isLength({ max: 20 }).withMessage("Numéro de téléphone invalide"),
  ],
  async (req, res) => {
    ...
    try {
      ...
      const user = await User.create({ name, email, password, role, studentId, phone: phone || "" });
      res.status(201).json({
        _id: user._id,
        name: user.name,
        email: user.email,
...
router.get("/users", protect, librarianOnly, async (req, res) => {
  try {
    const users = await User.find({}, "name email role studentId profileImage phone").lean();
    res.json(users);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      const { name, email, password, studentId, profileImage, phone } = req.body;
